...
* ONTAP SANドライバ*: iSCSIには指定しないでくださいAstra Tridentは、を使用してlink:https://docs.netapp.com/us-en/ontap/san-admin/selective-lun-map-concept.html["ONTAP の選択的LUNマップ"^]、マルチパスセッションの確立に必要なiSCI LIFを検出します。が明示的に定義されている場合は、警告が生成され `dataLIF`ます。
 a| 
`10.0.0.2`
...
| `aggregate`  a| 
プロビジョニング用のアグリゲート（オプション。設定する場合は SVM に割り当てる必要があります）。ドライバの場合 `ontap-nas-flexgroup`、このオプションは無視されます。SVM に割り当てられたすべてのアグリゲートを使用して FlexGroup ボリュームがプロビジョニングされます。
 a| 
`aggr1`
...
| `igroupName`  a| 
Astra Tridentでは、ノードごとにASを `netappdvp`作成、管理し `igroups`ます。
...
| `sanType` | *ドライバでのみサポートされてい `ontap-san`ます。*iSCSIまたは `nvme`NVMe/TCPの選択に使用し `iscsi`ます。 | `iscsi`空白の場合 
...
| `snapshotDir`  a| 
ディレクトリにアクセスするためのNASオプション `.snapshot`。デフォルトは `false`
 a| 
`true`
...
FlexGroupとFlexGroupに適したワークロードについては、を参照してください https://www.netapp.com/pdf.html?item=/media/12385-tr4571pdf.pdf["NetApp FlexGroup Volume Best Practices and Implementation Guide"^]。
