...
Astra Tridentは、クラスタと通信するためのドライバを提供します `gcp-cvs` 。サポートされているアクセスモードは、_ReadWriteOnce_(RWO)、_ReadOnlyMany_(ROX)、_ReadWriteMany_(RWX)、_ReadWriteOncePod_(RWOP)です。
...
Astra Tridentでは、link:https://cloud.google.com/architecture/partners/netapp-cloud-volumes/service-types["サービスタイプ"^]次の2つのいずれかにCloud Volumes Serviceボリュームを作成できます。
