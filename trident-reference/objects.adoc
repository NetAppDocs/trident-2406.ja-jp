...
NOTE: 通常、オブジェクトを管理する必要はありませ `VolumeSnapshotContent`ん。ただし、Astra Trident以外でを作成する場合は例外ですlink:../trident-use/vol-snapshots.html#import-a-volume-snapshot["ボリュームSnapshotのインポート"]。
