...
WARNING: この操作は元に戻すことはできません。Astra Tridentを完全に新規にインストールする場合を除き、この作業は行わないでください。CRDを削除せずにAstra Tridentをアンインストールする方法については、を参照してくださいlink:trident-managing-k8s/uninstall-trident.html["Astra Trident をアンインストール"]。
